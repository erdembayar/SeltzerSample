...
module.exports = {
	"class": L.Control.extend({
		...
		onAdd: function (map) {
		    ...
		    /*********************************************************************************
            * Duncan commented this out to change the behaviour of the search bar so that    *
            * when set to NOT collapse when initialised, it will still allow the toggle      *
            * to collapse back. Originally the code changes the behaviour: if not            *
            * collapsed on init, then clicking the collapse icon will run the geocode        *
            **********************************************************************************/
			//if (this.options.collapsed) {
		...
		_geocodeSuggestion: function (locationId) {
		    var requestCount = ++this._requestCount,
				mode = 'geocode';
		    ...
		    this.fire('start' + mode);
		    this.options.geocoder.geocodeSuggestion(locationId, function (results) {
		        if (requestCount === this._requestCount) {
		            this.fire('finish' + mode);
		            this._geocodeResult(results, false);
		        }
		    }, this);
		},
		...
		_processResult : function(result){
		    if (result.suggested) {
		        ...
		        this._geocodeSuggestion(result.locationId);
		    }
		    else {
		...
		_keydown: function(e) {
			...
			case 13:
				if (this._selection) {
				    ...
				    this._processResult(this._results[index]);
					//this._clearResults();
				} else {
					this._geocode();
				}
...
module.exports = {
    "class": L.Class.extend({
        options: {
            ...
            suggestUrl: 'https://autocomplete.geocoder.cit.api.here.com/6.2/suggest.json',
        ...
        suggest: function (query, cb, context) {
            ...
            params = L.Util.extend(params, this.options.suggestQueryParams);

            Util.getJSON(this.options.suggestUrl, params, function (data) {
                if (data && data.suggestions) {
                    var results = $.map(data.suggestions, function (item) {
                        return {
                            html: item.label,
                            name: item.label.replace(/(<strong>|<\/strong>)/gm, ''),
        ...
        geocodeSuggestion: function (locationId, cb, context) {
            var params = {
        ...
        reverse: function(location, scale, cb, context) {
            var params = {
                prox: encodeURIComponent(location.lat) + ',' + encodeURIComponent(location.lng) + ',100',
