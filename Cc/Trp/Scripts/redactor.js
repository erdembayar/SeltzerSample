...
(function ($) {
    ...
    $.Redactor.opts = {
        ...
        langs: {
            en: {
                ...
                delete_header_row: 'Delete Header Row',
                title: 'Title',
                ...
                edit: 'Edit',
                edit_link_url: 'Edit URL',
                edit_link_text: 'Edit text',
    ...
    Redactor.fn = $.Redactor.prototype = {
        ...
        init: function (el, options) {
            ...
            this.banBold = false;
            this.banItalics = false;
            this.banAnchors = false;
            this.banOrderedLists = false;
            this.banUnorderedLists = false;

            this.linkCallback = options.linkCallback;
            ...
            return {
                ...
                setEvents: function () {
                    ...
                    $(document).on('mousedown.redactor-blur.' + this.uuid, $.proxy(function (e) {
                        ...
                        var closest = $(e.target).closest('.redactor-editor, .redactor-toolbar, .redactor-dropdown');
                        if (closest.size() !== 0 && (closest.is(this.$editor) || closest.is(this.$toolbar) || closest.is(this.$dropdown))) {
                            return;
                        }
        ...
        inline: function () {
            return {
                setBoldEnabled: function (enabled) {
                    this.banBold = enabled;
                },
                setItalicsEnabled: function (enabled) {
                    this.banItalics = enabled;
                },
                formatting: function (name) {
                    var type, value;
                ...
                format: function (tag, type, value) {
                    if (this.banBold && tag === 'bold')
                        return;

                    if (this.banItalics && tag === 'italic')
                        return;
        ...
        keydown: function () {
            return {
                ...
                onShiftEnter: function (e) {
                    ...
                    if (this.opts.allowLineBreaksHack) {
                        ...
                        this.keydown.insertParagraph(e);
                    }
                },
        ...
        link: function () {
            return {
                setAnchorsEnabled: function(enabled) {
                    this.banAnchors = enabled;
                },
                show: function (e) {
                    if (this.banAnchors)
                        return;
                    ...
                    // Call function which returns promise containing URL
                    var promise = this.linkCallback(this.link.$node);

                    var self = this;

                    promise.done(function (linkData) {
                        // TODO
                        self.link.insert(linkData, self.link.text || 'Click to edit text');
                    });
                },
                cleanUrl: function () {
                ...
                insert: function (linkData, text) {
                    ...
                    this.link.set(text, linkData, target);
                    this.modal.close();
                },
                set: function (text, linkData, target) {
                    ...
                    var setAttrs = function ($elt) {
                        $elt.attr({
                            'data-target': linkData.target,
                            'data-id': linkData.id || null,
                            'data-site-id': linkData.siteId || null,
                            'data-library-id': linkData.libraryId || null,
                            'data-404': null,
                            'data-inactive': null
                        });
                    }
                    ...
                    if (this.link.$node) {
                        ...
                        if (this.utils.browser('mozilla') && this.link.text === '') {
                            var $a = $('<a />').attr('href', linkData.href).text(text);
                            if (target !== '')
                            ...
                            var $a;
                            if (this.utils.browser('msie')) {
                                $a = $('<a href="' + linkData.href + '">').text(text);
                                ...
                                document.execCommand('createLink', false, linkData.href);
                                ...
                                // We have to go through this rigmarole because $a doesn't actually always represent the link just inserted,
                                // thanks to dodgy redactor selection bugs.
                                this.$editor
                                    .find('a[href="' + linkData.href + '"]')
                                    .each(function () {
                                        var $a = $(this);
                ...
                format: function () {
                    ...
                    this.$editor
						...
						.each(function () {
                            ...
                            // If we don't have this hack here, Redactor will do something retarded and 'linkify' (aka render) the iframe,
                            // despite it not being on our whitelist. We do still want to support linkification in the general case, at least for now.
        ...
        list: function () {
            return {
                setOrderedListsEnabled: function(enabled) {
                    this.banOrderedLists = enabled;
                },
                setUnorderedListsEnabled: function (enabled) {
                    this.banUnorderedLists = enabled;
                },
                toggle: function (cmd) {
                    if ( (cmd === 'orderedlist' && this.banOrderedLists) || (cmd === 'unorderedlist' && this.banUnorderedLists) )
                        return;
        ...
        observe: function () {
            return {
                ...
                showTooltip: function (e) {
                    ...
                    var tooltip = $('<div class="redactor-link-tooltip"></div>');

                    var target = $link.attr('data-target'),
                        href = $link.attr('href');

                    if (href === undefined || href === null || target !== 'External') {
                        href = '';
                    }
                    ...
                    var aLink = href
                        ? $('<a href="' + $link.attr('href') + '" target="_blank" />').html(href).addClass('redactor-link-tooltip-action')
                        : '';

                    if (href === '') 
                        tooltip.addClass('no-render-link');
                    
                    var aUrlEdit = $('<a href="#" />').html(this.lang.get('edit_link_url'))
                        .on('click', $.proxy(this.link.show, this)).addClass('redactor-link-tooltip-action');
                    
                    var fourOhFourPane = '';
                    if ($link.attr('data-404') === 'true') {
                        var isPage = $link.attr('data-target') === 'Page';
                        
                        fourOhFourPane = $(
                            '<div class="four-oh-four-pane">' +
                                (isPage ? 'Broken page reference' : 'Broken file reference') +
                                ' - ' +
                                $link.attr('href') +
                            '</div>'
                        );
                    ...
                    var textInput = linkTextEditor.find('input[type="text"]');
                    textInput.addClass('redactor-link-tooltip-action');

                    var aTextEdit = $('<a href="#" />').html(this.lang.get('edit_link_text'))
                        .on('click', $.proxy(function () {
                            textInput.val($link.text());
                            linkTextEditor.show();

                            return false;
                        }, this));

                    var $save = linkTextEditor.find('input.save');
                    $save.on('click', function () {
                        $link.text(textInput.val() || 'Link text');
                        self.code.sync();
                    });

                    var aUnlink = $('<a href="#" />')
                        .html(this.lang.get('unlink'))
                        .on('click', $.proxy(this.link.unlink, this))
                        .addClass('redactor-link-tooltip-action');

                    tooltip.append(aLink).append(href ? ' | ' : '').append(aUrlEdit).append(' | ').append(aTextEdit).append(' | ')
                        .append(aUnlink).append(fourOhFourPane).append(linkTextEditor);
                    ...
                    // Needed, otherwise link clicks sometimes leak outside Redactor and trigger Angular state changes
                    e.preventDefault();
                    return false;
                },
            ...
            return {
                ...
                createMarkers: function () {
                    ...
                    if (this.range.collapsed === false) {
                        ...
                        // Fix for Chrome v58 taken from here:
        ...
        toolbar: function () {
            return {
						    ...
						    dropdown:
							{
							    p:
								{
								    ...
								    observe: {
								        ...
								        in: {
								            attr: {
								                'class': 'redactor-dropdown-link-selected',
								                'aria-disabled': true
								            }
								        }
								    }
                ...
                fixToolbar: function (scrollTop, boxTop) {
                ...
                observeScrollEnable: function (scrollTop, boxTop) {
                    // Figure out if we have scrolled past the end of this Redactor instance                    
                    var height = this.$editor.height(),
                        magicNumber = 100,
                        pastEnd = height !== 0 && (scrollTop - boxTop + magicNumber) > height;

                    // We only want to stickify the toolbar if this Redactor instance is active and visible
                    var shouldBeSticky = !pastEnd && this.focus.isFocused();
                    
                    if (shouldBeSticky)
                        this.toolbar.fixToolbar(scrollTop, boxTop);
                    else
                        this.toolbar.unfixToolbar();
                },
        ...
        utils: function () {
            return {
                ...
                disableBodyScroll: function () {
                    ...
                    $body.css('overflow', 'hidden').addClass('disabled-scrollbar');
                ...
                enableBodyScroll: function () {
                    $('html').css({ 'overflow': '', 'padding-right': '' }).removeClass('disabled-scrollbar');
